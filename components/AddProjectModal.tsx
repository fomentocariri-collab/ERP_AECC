...
export const AddProjectModal: React.FC<AddProjectModalProps> = ({ isOpen, onClose, onSave, existingProject }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      if (existingProject) {
        ...
        reset({ status: 'Planning', startDate: new Date().toISOString().split('T')[0], budget: '0' });
      }
    }
  ...
  const onSubmit = async (data: ProjectFormData) => {
    setIsSaving(true);
    try {
        const finalData = { ...data, endDate: data.endDate || null } as any;
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2">
                    ...
                    <textarea {...register('description')} rows={10} className={TEXTAREA_CLASS} placeholder="Detalhes completos do projeto..." />
                    {errors.description && <p className={ERROR_CLASS}>{errors.description.message as string}</p>}
                </div>
                <div>
                     <label className="block text-sm font-medium">Proponente</label>
                     <input {...register('proponent')} className={INPUT_CLASS} />
                ...
                <div>
                     ...
                     <input {...register('sponsor')} className={INPUT_CLASS} />
            ...
            <div className="flex justify-end gap-3 pt-6 border-t dark:border-gray-700 mt-4">
                <button type="button" onClick={onClose} disabled={isSaving} className="px-5 py-2.5 bg-gray-100 dark:bg-gray-700 rounded-lg">Cancelar</button>
