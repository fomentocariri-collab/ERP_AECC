            ...
            <div className="space-y-6">
                <div className="flex justify-between items-center">
                    <h2 className="text-2xl font-bold text-gray-800 dark:text-white flex items-center gap-2">
                        <Briefcase className="text-secondary-600" /> Projetos
                    </h2>
                    {canPerformActions && (
                ...
                {projects.length === 0 ? (
                    <div className="bg-white dark:bg-gray-800 rounded-xl p-10 text-center shadow-sm border border-gray-200 dark:border-gray-700">
                        ...
                        <p className="text-gray-500 dark:text-gray-400 mt-1">Gerencie os projetos da associação, acompanhe orçamentos e prazos.</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {projects.map(project => (
                            <div key={project.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 flex flex-col hover:border-secondary-400 transition-colors">
                                <div className="p-6 flex-1">
                                    <div className="flex justify-between items-start mb-4">
                                        <span className={`px-2 py-1 text-xs font-bold rounded-full uppercase ${getStatusBadge(project.status)}`}>
                                            {project.status === 'Planning' ? 'Planejamento' : project.status === 'Active' ? 'Em Andamento' : project.status === 'Completed' ? 'Concluído' : 'Cancelado'}
                                        </span>
                                        {canPerformActions && (
                                            <div className="flex gap-2">
                                                <button onClick={() => handleOpenEdit(project)} className="text-gray-400 hover:text-blue-500"><Edit size={16} /></button>
                                    ...
                                    <div className="space-y-2 text-sm text-gray-500 dark:text-gray-400">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="truncate">Prop: {project.proponent}</span>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <DollarSign size={14} className="text-secondary-500"/>
                                            <span>Orçamento: R$ {project.budget.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                                        </div>
                                    </div>
                                </div>
                                <div className="bg-gray-50 dark:bg-gray-700/50 p-4 border-t dark:border-gray-700 rounded-b-xl">
                                    <p className="text-xs text-gray-500 dark:text-gray-400 text-center">Patrocinador: <span className="font-medium text-gray-700 dark:text-gray-200">{project.sponsor}</span></p>
                                </div>
                            </div>
                        ))}
                    </div>
